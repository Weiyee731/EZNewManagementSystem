...
function mapDispatchToProps(dispatch) {
  return {
    ...
    CallUpdateStockStatus: (props) =>
      dispatch(GitAction.CallUpdateStockStatus(props)),
    CallViewContainer: (props) => dispatch(GitAction.CallViewContainer(props)),
    CallUpdateStockDetailByPost: (props) =>
      dispatch(GitAction.CallUpdateStockDetailByPost(props)),
    CallInsertStockByPost: (props) =>
      dispatch(GitAction.CallInsertStockByPost(props)),
...
class StockGoods extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.Stocks.length !== this.state.stockListing.length) {
      if (
        ...
        this.props.ReturnVal !== "0"
      ) {
        this.setState({
          stockListing: this.props.Stocks,
    ...
    if (
      !isArrayNotEmpty(this.state.stockFiltered) &&
      isArrayNotEmpty(this.props.Stocks)
    ) {
      const { Stocks } = this.props
      this.setState({
        stockFiltered:
          isStringNullOrEmpty(Stocks.ReturnVal) && Stocks.ReturnVal === 0
            ? []
            : Stocks,
      })
    }
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.AllContainer.length !== this.props.AllContainer.length) {
      if (
        this.props.AllContainer !== undefined &&
        this.props.AllContainer[0] !== undefined
      ) {
        this.setState({ options: this.props.AllContainer })
      } else {
      }
    ...
    if (isArrayNotEmpty(this.props.stockApproval)) {
      ...
      setTimeout(() => {
        this.setState({ CallResetSelected: false })
      }, 300)
    }

    if (
      this.state.stockFiltered === null &&
      isArrayNotEmpty(this.props.Stocks)
    ) {
      const { Stocks } = this.props
      this.setState({
    ...
    if (prevProps.Stocks.length !== this.props.Stocks.length) {
      if (
        ...
        console.log(
          "No result found, please try to refresh or check network connection"
        )
      }
    } else {
    }
    ...
    if (prevState.selectedRows !== this.state.selectedRows) {
      if (this.state.selectedRows !== undefined) {
        this.setState({
          ...
          ProductDimensionHeight:
            this.state.selectedRows.ProductDimensionHeight,
          ProductDimensionDeep: this.state.selectedRows.ProductDimensionDeep,
          AreaCode: this.state.selectedRows.AreaCode,
          Item: isStringNullOrEmpty(this.state.selectedRows.Item)
            ? "no item"
            : this.state.selectedRows.Item,
          AdditionalCharges: isStringNullOrEmpty(
            this.state.selectedRows.AdditionalCharges
          )
            ? "0"
            : this.state.selectedRows.AdditionalCharges,
          Remark: isStringNullOrEmpty(this.state.selectedRows.Remark)
            ? "no remark"
            : this.state.selectedRows.Remark,
  ...
  renderTableRows(data, index) {
    ...
    var dimension =
      ...
      data.ProductDimensionHeight
        ? (
            (data.ProductDimensionDeep / 100) *
            (data.ProductDimensionWidth / 100) *
            (data.ProductDimensionHeight / 100)
          ).toFixed(3)
        : ""
    ...
    if (data.Remark !== "-" || data.AdditionalCharges !== null) {
      fontcolor = "#FF0000"
    }
  ...
  changeTab = (key) => {
    switch (key) {
      case "All":
        ...
        this.props.CallFetchAllStock({ TRACKINGSTATUSID: 1 })
        this.setState({
          stockFiltered: this.props.Stocks ? this.props.Stocks : "",
          needCheckBox: true,
        })
        break
      ...
      case "Unchecked":
        this.props.CallResetStocks()
        this.props.CallFetchAllStock({ TRACKINGSTATUSID: 1 })
        const FilterArr = this.props.Stocks.filter(
          (searchedItem) => searchedItem.TrackingStatusID === 1
        )
      ...
      case "Checked":
        ...
        const FilterArr2 = this.props.Stocks.filter(
          (searchedItem) => searchedItem.TrackingStatusID === 2
        )
  ...
  onDateChange = (e, type) => {
    if (e !== "") {
      this.setState({
        ContainerDate: new Date(e).toLocaleDateString("en-GB"),
        datevalue: e,
      })
    } else {
    }
  ...
  onSearch(keywords, area) {
    const { stockFiltered, searchKeywords, searchCategory, searchArea } =
      this.state
    const { Stocks } = this.props
    ...
    if (
      ...
      this.setState({ isDataFetching: true, stockFiltered: null })
    } else {
      let tempList
  ...
  //       }
  //       this.setState({ stockFiltered: tempList })
  //       if (isArrayNotEmpty(tempList) && tempList.length === 1) {
  //         this.setState({
  //           openEditModal: !this.state.openEditModal,
  //           selectedRows: tempList[0],
  //         })
  //       }
  //     } else {
  //       if (searchCategory === "All" && areaSearchKeys !== "All") {
  ...
  handleCallback = (childData) => {
    ...
    if (
      childData.TrackingNumber !== null &&
      childData.TrackingNumber !== undefined
    ) {
    ...
    if (
      childData.ProductDimensionWidth !== null &&
      childData.ProductDimensionWidth !== undefined
    ) {
      this.setState({ ProductDimensionWidth: childData.ProductDimensionWidth })
    }
    if (
      childData.ProductDimensionHeight !== null &&
      childData.ProductDimensionHeight !== undefined
    ) {
      this.setState({
        ProductDimensionHeight: childData.ProductDimensionHeight,
      })
    }
    if (
      childData.ProductDimensionDeep !== null &&
      childData.ProductDimensionDeep !== undefined
    ) {
      this.setState({ ProductDimensionDeep: childData.ProductDimensionDeep })
    }
    if (
      childData.ProductWeight !== null &&
      childData.ProductWeight !== undefined
    ) {
    ...
    if (
      childData.AdditionalCharges !== null &&
      childData.AdditionalCharges !== undefined
    ) {
      ...
      <div className="container-fluid">
        ...
        {openEditModal && (
          <ModalPopOut
            ...
            handleConfirmFunc={() =>
              this.handleSearchfilter("openEditModal", checked)
            }
            message={
              <EditStockGoods
                data={this.state.selectedRows}
                ContainerDate={
                  this.state.ContainerDate ? this.state.ContainerDate : ""
                }
                ContainerName={
                  this.state.ContainerName ? this.state.ContainerName : ""
                }
          ...
          >
            <Tooltip title="Click to change container" placement="bottom-start">
              <div style={{ fontSize: "1.05rem" }}>
                <strong>Container Number and Date : </strong>
